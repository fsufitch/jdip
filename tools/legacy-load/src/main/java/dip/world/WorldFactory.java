...
public class WorldFactory {
    ...
    public World createWorld(Variant variant) {
        ...
        for (int i = 0; i < provinceDataArray.length; i++) {
            ...
            if (!isUnique(provNameMap, provinceData.getFullName(), shortNames)) {
                throw new RuntimeException(WF_PROV_NON_UNIQUE + provinceData.getFullName());
            }
        ...
        for (ProvinceData provinceData : provinceDataArray) {
            ...
            if (adjProvinceTypes.length != adjProvinceNames.length) {
                throw new RuntimeException(WF_PROV_MISMATCH);
            }
            ...
            if (!adjacency.validate(province)) {
                throw new RuntimeException(WF_ADJ_INVALID + provinceData.getFullName());
            }
        ...
        // set the Border data (if any) for each province.
        {
            ...
            for (ProvinceData provinceData : provinceDataArray) {
                ...
                for (String borderName : borderNames) {
                    ...
                    if (border == null) {
                        throw new RuntimeException(WF_BAD_BORDER_NAME + province.getShortName() + borderName);
                    }
        ...
        if (phase == null) {
            throw new RuntimeException(WF_BAD_STARTINGTIME);
        }
        ...
        for (SupplyCenter supplyCenter : supplyCenters) {
            ...
            if (province == null) {
                throw new RuntimeException(WF_BAD_SC_PROVINCE + supplyCenter.getProvinceName());
            }
            ...
            if (!"none".equalsIgnoreCase(hpName)) {
                ...
                if (power == null) {
                    throw new RuntimeException(WF_BAD_SC_HOMEPOWER + hpName);
                }
            ...
            if (!"none".equalsIgnoreCase(scOwner)) {
                ...
                if (power == null) {
                    throw new RuntimeException(WF_BAD_SC_OWNER + scOwner);
                }
        ...
        for (InitialState initState : initStates) {
            ...
            if (power == null) {
                throw new RuntimeException((WF_BAD_IS_POWER));
            }

            // n/a if we use a validating parser
            if (province == null) {
                throw new RuntimeException((WF_BAD_IS_PROVINCE));
            }
            ...
            if (unitType != null) {
                ...
                throw new RuntimeException((WF_BAD_IS_UNIT + initState.getProvinceName()));
            }
        }
        ...
        if (variant.getNumSCForVictory() <= 0
            ...
            throw new RuntimeException(WF_BAD_VC);
        }
    ...
    private Location makeLocation(HashMap provNameMap, String name, Coast theDefaultCoast)
        ...
        if (defaultCoast.equals(Coast.UNDEFINED)) {
            throw new RuntimeException((WF_ADJ_BAD_TYPE + name));
        ...
        if (province == null) {
            throw new RuntimeException(WF_ADJ_BAD_PROVINCE + name + provinceName + defaultCoast);
        }
    ...
    private Location[] makeBorderLocations(String in, HashMap provNameMap)
        ...
        while (st.hasMoreTokens()) {
            ...
            if (province == null) {
                throw new RuntimeException(WF_BAD_BORDER_LOCATION + tok);
            }
